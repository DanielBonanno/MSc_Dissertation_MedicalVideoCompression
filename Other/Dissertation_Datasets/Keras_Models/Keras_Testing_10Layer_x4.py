#Arguments - LR Path, MC path, HR save Path
...
LR_images_file_path = sys.argv[1];
MC_images_file_path = sys.argv[2];
output_image_file_path = sys.argv[3];
...
def generate_data(filepath, batch_size):
	...
	while True:
		...
		#yield ([np.array(data_LR_batch)], [np.array(data_HR_batch)])	
...
def Mix_Loss(y_true, y_pred):
	...
	return (0.9*msssim_loss+0.1*L1_loss)
...
weights_path = './Weights/MVSRCNN_M10_x4.h5'

model.load_weights(weights_path)
model.pop()

model.compile(optimizer=Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=1e-8),loss='MSE', metrics = ['mse','mae', SSIM])

#-----------------------------------------------------------


all_images = os.listdir(LR_images_file_path)
for image in all_images:		#for every image

	LR_image_path = LR_images_file_path+'/'+image
	MC_image_path = MC_images_file_path+'/'+image
	Save_image_path = output_image_file_path+'/'+image
	
	#original_image = cv2.imread(original_image_path,0)/255.0
	#o_width, o_height =  original_image.shape
	#print original_image.shape			
	#original_image =  np.reshape(original_image, (1,height,width,1))


	LR_image = cv2.imread(LR_image_path, 0)/255.0
	width, height = LR_image.shape
	LR_image = np.reshape(LR_image, (1,width,height,1))


	MC_image = cv2.imread(MC_image_path, 0)/255.0
	MC_image = np.reshape(MC_image, (1,width,height,1))/255.0
	
	start = time.time()
	output = model.predict([LR_image, MC_image])
	end = time.time()
	print end-start

	output = np.reshape(output, (width*4, height*4))
	
	cv2.imwrite(Save_image_path, output*255.0);
