...
cd "../../../Dissertation_Code/Image_Subsample_Padded"
exe_path=$(pwd)
...
for set in */; do	#Iterate over every set in the directory
	...
	mkdir -p -m 777 "Padded"
	padded_dir="$(pwd)/Padded"
	cd "Original"
	frames_dir="$(pwd)"
	cd "../"
	mkdir -p -m 777 "x$3"
	...
	for frame in $frames_dir/*.bmp; do	#for every bmp file
		...
		$exe_path/main $frame $padded_dir/$frame_filename $subsampled_frames_dir/$frame_filename $3
	done
